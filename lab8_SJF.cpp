...
int main()
{
    ...
    freopen("input_non_preemptable.txt", "r", stdin); //set input.txt as the stdin
    ...
    */
    ...
    for (int i = 0; i < waitvector.size(); i++)
    {
        if (waitvector[i].period == 0)
            continue;
        //hyper_period = LCM(hyper_period, waitvector[i].period);   lab8 don't have period
    }
    ...
    while (1)
    {
        ...
        if (process_time == hyper_period && 0)
            break;
        ...
        sort(readyvector_low.begin(), readyvector_low.end(), sort_execution_time);
        sort(waitvector.begin(), waitvector.end(), sort_realease_time);
        ...
        if (waitvector.size() > 0)
        {
            ...
            while (waitvector.begin()->release_time <= process_time && waitvector.size() > 0)
            {
                ...
                sort(readyvector_low.begin(), readyvector_low.end(), sort_execution_time); //sort the ready queue by deadline
        ...
        for (int i = 0; i < processor_num; i++)
        {
            ...
            if (runvector[i].size() <= 0)
            {
                ...
                else if (readyvector_low.size() > 0)
                {
                    ...
                    sort(readyvector_low.begin(), readyvector_low.end(), sort_execution_time); //sort ready queue with deadline
            ...
            {
                //context switch with shortest execution time  //lab8 SJF
                ...
                if (get_slack_time(runvector[i][0]) > get_slack_time(readyvector_low[0]) && 0)
                {
                    runvector[i][0].suspend = 1;                //set context switch flag
                    readyvector_low.push_back(runvector[i][0]); //move running task to ready queue
                    runvector[i].erase(runvector[i].begin());   //remove running task
